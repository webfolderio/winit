...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        WM_NCCALCSIZE => {
            ...
            // The client covers the whole window. An undecorated shadow comes from the frame
            // `WindowFlags::apply_undecorated_shadow_frame` extends under it instead of from a
            // strip of non-client area, which Windows 10 paints as a 1px line along the top.
        ...
        WM_POINTERDOWN | WM_POINTERUPDATE | WM_POINTERUP => {
            ...
            if let (Some(GetPointerFrameInfoHistory), Some(SkipPointerFrameMessages)) =
                (*util::GET_POINTER_FRAME_INFO_HISTORY, *util::SKIP_POINTER_FRAME_MESSAGES)
            {
